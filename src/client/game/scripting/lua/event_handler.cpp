...
namespace scripting::lua
{
	...
	void event_handler::dispatch(const event& event)
	{
		...
		callbacks_.access([&](task_list& tasks)
		{
			...
			{
				if (i->entity != event.entity || (i->event != event.name 
					&& event.string_value != game::Dvar_GenerateHash(i->event.data())))
			...
			for (auto& task : tasks)
			{
				for (auto& condition : task.endon_conditions)
				{
					if (condition.first == event.entity && (condition.second == event.name 
						|| event.string_value == game::Dvar_GenerateHash(condition.second.data())))
					{
						task.is_deleted = true;
						break;
					}
